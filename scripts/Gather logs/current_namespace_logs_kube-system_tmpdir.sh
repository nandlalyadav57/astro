...
export ASTRONOMER_NAMESPACE=kube-system
export DIR="/tmp/gen2_logs"
...
cd "$DIR"
cd ..
tar -czvf "gen2_logs"_$(date +"%Y_%m_%d_%I_%M_%p").tar.gz "$DIR"
zip -r "gen2_logs"_$(date +"%Y_%m_%d_%I_%M_%p").zip "$DIR"
cdir=$PWD
